  ...
  return useMemo(() => {
    const microSteps: Array<{
    ...
    }> = [
      {
        ...
        anchor: "#results-step-rubric"
      },
      {
        label: "Tiến độ chấm",
        detail: scoringComplete ? "Phiếu đã nộp đủ" : "Theo dõi GK nộp phiếu",
        state: !hasRubric ? "blocked" : scoringComplete ? "done" : "active",
    ...
    if (!hasBoards) {
      ...
      nextAction = {
        ...
        description: "Thiết lập rubric trước khi giám khảo chấm.",
        href: "#results-step-rubric",
        cta: "Đi tới rubric"
      };
    ...
    const setupComplete = hasPublished && (!showFinalsStep || finalsDone);
